...
# 카드 개인  청구내역 조회
billing_list_path = '/v1/kr/card/p/account/billing-list'
...
# CODEF API 요청
response_codef_api = http_sender(CODEF_URL + billing_list_path, token, body)
...
# token error
    ...
    if response_oauth.code == '200'
        ...
        # request codef_api
        response = http_sender(CODEF_URL + billing_list_path, token, body)
    else
        puts('토큰발급 오류')
    end
else
