...
# 카드 법인 한도조회
limit_path_path = '/v1/kr/card/b/account/limit'
...
# CODEF API 요청
response_codef_api = http_sender(CODEF_URL + limit_path_path, token, body)
...
# token error
    ...
    if response_oauth.code == '200'
        ...
        # request codef_api
        response = http_sender(CODEF_URL + limit_path_path, token, body)
    else
        puts('토큰발급 오류')
    end
else
